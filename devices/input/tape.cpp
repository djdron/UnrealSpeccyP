...
dword eTape::FindPulse(dword t)
{
   ...
   tape_err = 1;
   dword nearest = 0; int delta = 0x7FFFFFFF;
...
{
   for (dword i = 0; i < tape_infosize; i++) {
      dword end = (i == tape_infosize-1) ? tape_imagesize : tapeinfo[i+1].pos;
      dword sz = 0;
...
#define WORD4(a,b,c,d) (((dword)(a))+((dword)(b))*0x100+((dword)(c))*0x10000+((dword)(d))*0x1000000)
...
void eTape::Reserve(dword datasize)
{
   ...
   if (!tape_image) tape_image = (byte*)malloc(newsize);
   if (align_by(tape_imagesize, blocksize) < newsize) tape_image = (byte*)realloc(tape_image, newsize);
}

void eTape::MakeBlock(byte*data, dword size, dword pilot_t,
      dword s1_t, dword s2_t, dword zero_t, dword one_t,
      dword pilot_len, dword pause, byte last)
{
...
{
   byte crc = 0; char prg[10];
   dword i; //Alone Coder 0.36.7
   for (/*dword*/ i = 0; i < size; i++) crc ^= data[i];
...
void eTape::CreateAppendableBlock()
{
   ...
   NamedCell("set of pulses"); appendable = 1;
}
   ...
   while(ptr < buf + buf_size)
   {
	 ...
	 case 0x11: // turbo block
		...
		MakeBlock(ptr + 0x12, size,
		   *(word*)ptr, *(word*)(ptr+2),
		   *(word*)(ptr+4), *(word*)(ptr+6),
		   *(word*)(ptr+8), *(word*)(ptr+10),
		   *(word*)(ptr+13), ptr[12]);
	 ...
	 case 0x21: // group start
		n = *ptr++;
		NamedCell(ptr, n); ptr += n;
		appendable = 1;
		break;
	 case 0x22: // group end
		break;
	 case 0x23: // jump to block
		NamedCell("* jump"); ptr += 2;
		break;
	 case 0x24: // loop start
		loop_n = *(word*)ptr; loop_p = tape_imagesize; ptr += 2;
		break;
	 case 0x25: // loop end
		if (!loop_n) break;
		...
		break;
	 case 0x26: // call
		NamedCell("* call"); ptr += 2 + 2 * *(word*)ptr;
		break;
	 case 0x27: // ret
		NamedCell("* return");
		break;
	 case 0x28: // select block
		...
		for (i = 0; i < n; i++) {
		   if (i) strcat(nm, " / ");
		   char *q = nm+strlen(nm); size = *(byte*)(p+2);
		   memcpy(q, p+3, size); q[size] = 0; p += size+3;
		}
		NamedCell(nm); ptr += 2 + *(word*)ptr;
		break;
	 ...
	 case 0x34: // emulation info
		NamedCell("* emulation info"); ptr += 8;
		break;
	 case 0x35: // custom info
		if (!memcmp(ptr, "POKEs           ", 16)) {
		   ...
		   for (i = 0; i < n; i++) {
			  ...
			  for (j = 0; j < t; j++) {
				 ...
				 sprintf(nm+strlen(nm), *p & 0x10 ? "nn" : "%d", *(byte*)(p+3));
		...
		break;
	 case 0x40: // snapshot
		NamedCell("* snapshot"); ptr += 4 + (0xFFFFFF & *(dword*)(ptr + 1));
		break;
	 case 0x5A: // 'Z'
		ptr += 9; break;
	 default:
...
void fast_tape()
{
   byte*ptr = am_r(cpu.pc);
   dword p = *(dword*)ptr;
   if (p == WORD4(0x3D,0x20,0xFD,0xA7)) { // dec a:jr nz,$-1
      cpu.t += ((byte)(cpu.a-1))*16; cpu.a = 1;
      return;
   }
   if ((dword short)p == WORD2(0x10,0xFE)) { // djnz $
      cpu.t += ((byte)(cpu.b-1))*13; cpu.b = 1;
      return;
   }
   if ((dword short)p == WORD2(0x3D,0xC2) && (cpu.pc & 0xFFFF)==(p>>16)) { // dec a:jp nz,$-1
      cpu.t += ((byte)(cpu.a-1))*14; cpu.a = 1;
      return;
   }
   if ((p | WORD4(0,0,0,0xFF)) == WORD4(0x04,0xC8,0x3E,0xFF)) {
      if (*(dword*)(ptr+4) == WORD4(0xDB,0xFE,0x1F,0xD0) &&
          *(dword*)(ptr+8) == WORD4(0xA9,0xE6,0x20,0x28) && ptr[12] == 0xF3)
      ...
      if (*(dword*)(ptr+4) == WORD4(0xDB,0xFE,0xCB,0x1F) &&
          *(dword*)(ptr+8) == WORD4(0xA9,0xE6,0x20,0x28) && ptr[12] == 0xF3)
      ...
      if (*(dword*)(ptr+4) == WORD4(0xDB,0xFE,0x1F,0x00) &&
          *(dword*)(ptr+8) == WORD4(0xA9,0xE6,0x20,0x28) && ptr[12] == 0xF3)
      ...
      if (*(dword*)(ptr+4) == WORD4(0xDB,0xFE,0xA9,0xE6) &&
          *(dword*)(ptr+8) == WORD4(0x40,0xD8,0x00,0x28) && ptr[12] == 0xF3)
      ...
      if (*(dword*)(ptr+4) == WORD4(0xDB,0xFE,0x1F,0xA9) &&
          *(dword*)(ptr+8) == WORD4(0xE6,0x20,0x28,0xF4))
   ...
   if ((p | WORD4(0,0,0,0xFF)) == WORD4(0x04,0x20,0x03,0xFF) &&
        ptr[6] == 0xDB && *(dword*)(ptr+8) == WORD4(0x1F,0xC8,0xA9,0xE6) &&
        (*(dword*)(ptr+0x0C) | WORD4(0,0,0,0xFF)) == WORD4(0x20,0x28,0xF1,0xFF))
   ...
   if ((p | WORD4(0,0xFF,0,0)) == WORD4(0x3E,0xFF,0xDB,0xFE) &&
       *(dword*)(ptr+4) == WORD4(0xA9,0xE6,0x40,0x20) &&
       (*(dword*)(ptr+8) | WORD4(0xFF,0,0,0)) == WORD4(0xFF,0x05,0x20,0xF4))
   { // lode runner
      for (;;) {
         if (cpu.b == 1) return;
         if ((tape_bit() ^ cpu.c) & 0x40) return;
